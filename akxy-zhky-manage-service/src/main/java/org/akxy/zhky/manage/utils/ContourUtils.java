...
public class ContourUtils {
	...
	public static List<Polygon> genereateContour(
			...
			double[] clipXs, double[] clipYs, double[] isolineLevel,int color) throws Exception {
		...
		List<PolyLine> contourLines = new ArrayList<>();
		List<Polygon> contourPolygons = new ArrayList<>();
		...
		if(color == 1){
			...
			for(int i = 0 ;i < contourPolygons.size();i++){
				RGB rgb = getRGB3Layer(contourPolygons.get(i).LowValue, isolineLevel[0], isolineLevel[1]);
				contourPolygons.get(i).setRGB(new int[]{rgb.r,rgb.g,rgb.b});
			}
		}
	...
	//蓝、绿、黄、红
	public static RGB getRGB3Layer(double Value, double Yellow, double Red){
	...
	public static RGB getRGB(double Value, double Yellow, double Red){
		...
		{
			if (Value < Yellow/255.0)
			{
				//起步蓝色
				return new RGB(0,0,255);
			}

			else if (Value >= Yellow/255.0 && Value < Yellow*205/255)
			{
				for (int i = 1 ; i < 205 ; i++)
				{
					if (Value >= i*Yellow/255.0 && Value < (i+1)*Yellow/255.0)
					{
						//减弱蓝色
						return new RGB(0,i,255);
					}
			...
			else if (Value >= Yellow*205/255.0 && Value < Yellow)
			{
				for (int i = 1 ; i < 50 ; i++)
				{
					if (Value >= (i + 205)*Yellow/255.0 && Value < (i+206)*Yellow/255.0)
					{
						//减弱蓝色
						return new RGB(0,205+i,255);
					}
				}
			}
			...
			else if (Value <= Red + 3)
			{
				for (int i = 0; i < 50 ; i++)
				{
					if (Value >= i*(Red + 3)/50.0 + Red && Value < (i+1)*(Red + 3)/50.0 + Red)
					{
						return new RGB(0,255,155-i);
					}
				}
			}
		}
